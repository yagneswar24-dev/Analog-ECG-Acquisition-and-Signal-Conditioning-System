...
ISR(TIMER1_COMPA_vect) {
  // 1. Generate the fake simulated noise
  int16_t current_noise = noise_50Hz[noise_index];
  if (++noise_index >= 10) noise_index = 0;

  // 2. READ THE PHYSICAL NOISE FROM A3 IN REAL-TIME
  // Taking a reading here ensures 0ms lag between measurement and cancellation
  int rawA3 = analogRead(A3); 
  
  // Convert 10-bit reading (0-1023) to an 8-bit signed value (-128 to +127)
  // E.g., if A3 reads 512 (baseline), measured_noise = 0.
  int16_t measured_noise = (rawA3 / 4) - 128;
  ...
  // DIRECT SUBTRACTION MATH:
  // (Baseline) + (ECG Signal) + (Simulated Noise) - (Measured Noise from A3)
  int16_t out_A = 128 + ecg_A_val + current_noise - measured_noise;
  if (out_A > 255) out_A = 255; else if (out_A < 0) out_A = 0; 
  OCR2B = (uint8_t)out_A;
  ...
  // DIRECT SUBTRACTION MATH:
  // (Baseline) + (ECG Signal) + (Simulated Noise) - (Measured Noise from A3)
  int16_t out_B = 128 + ecg_B_val + current_noise - measured_noise;
  if (out_B > 255) out_B = 255; else if (out_B < 0) out_B = 0; 
  OCR2A = (uint8_t)out_B;

  // --- RLD OUTPUT (PIN 5) --- 
  int16_t out_RLD = 128 - current_noise;
  if (out_RLD > 255) out_RLD = 255; else if (out_RLD < 0) out_RLD = 0; 
  OCR0B = (uint8_t)out_RLD;
}
...
void loop() {
  // Now the loop only handles plotting! The ISR handles all real-time cancellation.
  ...
  float voltsA2 = (rawA2 * 5.0) / 1023.0;

  Serial.print("Physically_Cleaned_A(V):");
  Serial.print(voltsA);
  Serial.print(",");
  Serial.print("Physically_Cleaned_B(V):");
  Serial.print(voltsB);
  Serial.print(",");
  Serial.print("Hardware_Output_A2(V):");
  Serial.println(voltsA2);
